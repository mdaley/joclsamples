...
public class SimpleConvolution
{
    ...
    private static BufferedImage createBufferedImage(String fileName)
    {
        ...
        try
        {
            image = ImageIO.read(SimpleConvolution.class.getResourceAsStream("/lena512color.png"));
        }
        catch (IOException e)
        {
            e.printStackTrace();
...
class JOCLConvolveOp implements BufferedImageOp
{
    ...
    private static String readFile(String fileName)
    {
        try
        {
            BufferedReader br = new BufferedReader(new InputStreamReader(SimpleConvolution.class.getResourceAsStream(fileName)));
